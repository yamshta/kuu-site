...
本アプリの音声認識（文字起こし）および AI 分類（カテゴリ分け）は、すべて**お使いの iOS デバイス上で完結**します。

- 音声認識：Apple の Speech フレームワーク（オンデバイス）を使用します
- AI 分類：Apple の FoundationModels（オンデバイス）を使用します

話した内容、文字起こし結果、整理結果のいずれも、本アプリの開発者または第三者のサーバーに送信されることはありません。
...
本アプリは、機能提供のために以下の外部サービスを利用します。**話した内容・文字起こし・分類結果は、いずれのサービスにも送信されません**。

- **iCloud / CloudKit**（Apple 提供。あなた自身のプライベートデータベースにのみ保存・同期）
- **Speech フレームワーク**（Apple 提供。完全に端末上で実行）
- **FoundationModels**（Apple 提供。完全に端末上で実行）
...
本アプリは、独自サーバーや認証サービスを使用しません。アクセス解析・クラッシュレポートはユーザーの opt-in を前提としてのみ動作します。
...
Speech recognition (transcription) and AI organization (categorization) are performed **entirely on your iOS device**:

- Speech recognition: Apple's Speech framework (on-device)
- AI organization: Apple's FoundationModels (on-device)

None of your spoken content, transcribed text, or organized results is ever sent to the developer's servers or any third-party server.
...
The App uses the following external services. **Your spoken content, transcripts, and organized results are not sent to any of them.**

- **iCloud / CloudKit** (Apple, only your own private database)
- **Speech framework** (Apple, runs entirely on-device)
- **FoundationModels** (Apple, runs entirely on-device)
...
The App does not operate its own server and does not use authentication services. Analytics and crash reporting are gated on user opt-in.
