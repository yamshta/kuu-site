...
**ひとことで：** あなたが話したことはあなたのものです。**音声そのものは外部に送信しません**。文字起こしは端末の中で行います。整理（AI 分類）には既定で外部の AI（開発者のサーバー経由の Google Vertex AI）を使いますが、送られるのは文字にした内容だけで、整理にのみ使われ、サーバーにも AI にも保存されません（設定「オンデバイス」で端末内のみの整理に変更できます）。保存先はあなたの端末と iCloud（プライベートデータベース）だけです。開発者は内容を保存せず、受け取った内容を見ることもありません。いつでもアプリ内から全データを削除できます。アプリは Apple の課金（StoreKit）と Google AdMob の広告のために**必要最小限の通信**のみ行い、その情報には話した内容は含まれません（広告は KUU+ 加入で無効化できます）。アプリの品質改善と本番事故の即知化のために、**ユーザーが明示的に opt-in した場合に限り** Firebase Analytics / Crashlytics を利用しますが、こちらも話した内容は含みません（詳細は第 14 条）。
...
- 送信先は開発者が運用するサーバー（Google Cloud Run）で、そこから Google Vertex AI（Gemini）が分類を行います
- 送信された内容は**分類にのみ使用され、サーバーにもログにも保存されません**。Vertex AI の規約により AI の学習にも使用されません
- 設定の「オンデバイス」から、**端末内のみの分類**（Apple の FoundationModels / 端末内ルール）へいつでも変更できます。この場合、文字起こしテキストも端末の外に送信されません
...
- **kuu-api / Google Vertex AI（Gemini）**（既定の AI 分類。文字起こしテキストのみ送信。分類にのみ使用され、保存もされず、AI の学習にも使用されません。設定「オンデバイス」でオフにできます。詳細は第 3 条）
- **FoundationModels**（Apple 提供。完全に端末上で実行。「オンデバイス」設定時と、外部 AI が使えないときのフォールバックで使用）
...
**In short:** What you spoke is yours. **Your spoken content is not sent outside your device.** Speech transcription and AI organization happen entirely on your iOS device, and the result is stored only in your own iCloud private database. The developer has no access. You can delete everything from inside the app at any time. The app uses **minimal networking** only for Apple StoreKit (KUU+ subscription) and Google AdMob ads — your spoken content is never sent to either (ads disappear with KUU+). For app quality improvement and immediate crash awareness, Firebase Analytics / Crashlytics are used **only if you explicitly opt in**; your spoken content is never sent there either (see Article 14).
...
- It is sent to a server operated by the developer (Google Cloud Run), where Google Vertex AI (Gemini) performs the categorization.
- The sent text is **used only for categorization and is never stored** on the server or in logs. Under Vertex AI terms, it is not used to train the AI either.
- You can switch to **on-device-only organization** (Apple's FoundationModels / on-device rules) anytime via "On-device" in Settings. In that case, the transcript never leaves the device.
...
- **kuu-api / Google Vertex AI (Gemini)** (default AI organization. Only the transcript text is sent; used solely for categorization, never stored, never used for AI training. Can be turned off via "On-device" in Settings.)
- **FoundationModels** (Apple, runs entirely on-device. Used when "On-device" is enabled, or as fallback.)
