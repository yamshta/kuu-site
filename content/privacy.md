...
**In short:** What you spoke is yours. **Your voice itself is never sent outside your device.** Transcription happens on your device. AI organization uses an external AI by default, but only the transcribed text is sent. It is used solely for organizing and is not retained by the AI provider. You can switch to on-device-only organization via "On-device" in Settings. What is stored lives only on your device and in your iCloud (private database). The developer does not store your content and cannot view what is received. You can delete all data from inside the app at any time. The app makes only **minimal network requests** for Apple's billing (StoreKit) and Google AdMob ads, and that information never includes what you said (ads disappear with KUU+). For app quality improvement and immediate awareness of production incidents, Firebase Analytics / Crashlytics are used **only if you explicitly opt in**, and this too never includes what you said (see Article 14).
