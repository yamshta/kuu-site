...
- /privacy/index.html (ja) and /<locale>/privacy/index.html — from content/privacy/<code>.md
  (source of truth: content/privacy/ja.md. en is hand-authored; the other locales are
  regenerated by scripts/translate_privacy.py whenever ja.md changes)
...
LOCALES = {
    "ja": {
        ...
        "privacy_body": "音声の聞きとりは、iPhone ではすべて端末の中だけ。KUU が声そのものを残したり、送ったりすることはありません。Apple Watch では Watch 標準の入力を使い、KUU は文字だけを受け取ります。整理には AI を使い、送るのは文字にした内容だけ。それも保存されません。話したことは、あなたの iCloud（プライベート）にだけ同期されます。",
        # Closing
    ...
    "en": {
        ...
        "privacy_body": "On iPhone, listening happens entirely on your device — KUU never keeps or sends your voice. On Apple Watch, KUU uses the standard watch input and receives only text. Sorting uses AI, so only the text is sent, and it is never kept. Your notes sync only to your private iCloud.",
    ...
    "es": {
        ...
        "privacy_body": "En el iPhone, escuchar ocurre por completo en tu dispositivo: KUU nunca guarda ni envía tu voz. En el Apple Watch, KUU usa la entrada estándar del reloj y solo recibe texto. La organización usa IA, así que solo se envía el texto, y este nunca se guarda. Tus notas se sincronizan solo con tu iCloud privado.",
    ...
    "ko": {
        ...
        "privacy_body": "iPhone에서는 듣기가 모두 기기 안에서만 이루어지며, KUU가 음성 자체를 남기거나 보내는 일은 없습니다. Apple Watch에서는 워치 기본 입력을 사용하며, KUU는 텍스트만 받습니다. 정리에는 AI를 사용하므로 텍스트만 보내며, 그것도 남지 않습니다. 말한 것은 당신의 iCloud(프라이빗)에만 동기화됩니다.",
    ...
    "zh-Hans": {
        ...
        "privacy_body": "在 iPhone 上，聆听全都只在你的设备里完成——KUU 从不保留或发送语音本身。在 Apple Watch 上，KUU 使用手表的标准输入，只接收文字。整理会使用 AI，因此只发送文字内容，而且不会保留。记录只同步到你私人的 iCloud。",
    ...
    "de": {
        ...
        "privacy_body": "Auf dem iPhone geschieht das Zuhören vollständig auf deinem Gerät – KUU speichert und sendet deine Stimme selbst nie. Auf der Apple Watch nutzt KUU die Standardeingabe der Uhr und erhält nur Text. Zum Sortieren kommt KI zum Einsatz, gesendet wird nur der verschriftlichte Inhalt – und auch der wird nicht gespeichert. Was du gesprochen hast, wird ausschließlich mit deiner privaten iCloud synchronisiert.",
    ...
    "it": {
        ...
        "privacy_body": "Su iPhone il riconoscimento vocale avviene interamente sul dispositivo: KUU non conserva né invia mai la tua voce. Su Apple Watch, KUU usa l'input standard dell'orologio e riceve solo testo. Per l'ordinamento usiamo l'AI, quindi inviamo solo il testo trascritto, e nemmeno quello viene conservato. Ciò che dici si sincronizza solo con il tuo iCloud privato.",
    ...
    "vi": {
        ...
        "privacy_body": "Trên iPhone, việc nhận diện giọng nói diễn ra hoàn toàn trong thiết bị — KUU không bao giờ lưu lại hay gửi đi giọng nói của bạn. Trên Apple Watch, KUU dùng cách nhập chuẩn của đồng hồ và chỉ nhận phần chữ. Việc sắp xếp có dùng AI, nhưng chỉ gửi đi phần nội dung đã chuyển thành chữ, và phần đó cũng không được lưu. Những gì bạn nói chỉ được đồng bộ vào iCloud riêng tư của bạn.",
    ...
    "zh-Hant": {
        ...
        "privacy_body": "在 iPhone 上，聆聽全都只在你的裝置裡完成——KUU 從不保留、也不會傳送語音本身。在 Apple Watch 上，KUU 使用手錶的標準輸入，只接收文字。整理會用到 AI，因此只傳送轉成文字的內容，而且同樣不會保存。你說過的話，只會同步到你私人的 iCloud。",
    ...
    "nl": {
        ...
        "privacy_body": "Op de iPhone gebeurt spraakherkenning volledig op je eigen apparaat — KUU bewaart of verzendt je stem zelf nooit. Op de Apple Watch gebruikt KUU de standaardinvoer van het horloge en ontvangt alleen tekst. Voor het sorteren gebruiken we AI, dus wordt alleen de tekst verzonden, en ook die wordt niet bewaard. Wat je hebt gezegd, wordt alleen gesynchroniseerd met jouw eigen, privé iCloud.",
    ...
    "id": {
        ...
        "privacy_body": "Di iPhone, pengenalan suara sepenuhnya terjadi di dalam perangkat Anda — KUU tidak pernah menyimpan maupun mengirim suara Anda. Di Apple Watch, KUU memakai input standar jam tangan dan hanya menerima teks. Untuk memilah, KUU memakai AI, sehingga yang dikirim hanya teks hasil ucapan, dan itu pun tidak disimpan. Apa yang Anda bicarakan hanya disinkronkan ke iCloud pribadi Anda.",
    ...
    "ms": {
        ...
        "privacy_body": "Pada iPhone, pengecaman suara berlaku sepenuhnya di dalam peranti anda — KUU tidak pernah menyimpan atau menghantar suara anda. Pada Apple Watch, KUU menggunakan input standard jam tangan dan hanya menerima teks. Penyusunan menggunakan AI, jadi hanya teks yang dihantar, dan itu juga tidak disimpan. Apa yang anda cakapkan hanya disegerakkan ke iCloud peribadi anda.",
    ...
    "da": {
        ...
        "privacy_body": "På iPhone foregår lytningen udelukkende på din enhed — KUU gemmer eller sender aldrig din stemme. På Apple Watch bruger KUU urets standardindtastning og modtager kun tekst. Sorteringen bruger AI, så det er kun teksten, der bliver sendt, og heller ikke den bliver gemt. Det, du har sagt, synkroniseres udelukkende til din private iCloud.",
    ...
    "nb": {
        ...
        "privacy_body": "På iPhone skjer talegjenkjenningen helt og holdent på enheten din — KUU lagrer eller sender aldri stemmen din. På Apple Watch bruker KUU urets standardinntasting og mottar bare tekst. Sorteringen bruker KI, så det er bare teksten som sendes, og heller ikke den blir lagret. Det du har sagt, synkroniseres bare til din private iCloud.",
    ...
    "sv": {
        ...
        "privacy_body": "På iPhone sker taligenkänningen helt på enheten — KUU sparar eller skickar aldrig själva rösten. På Apple Watch använder KUU klockans standardinmatning och tar bara emot text. Sortering sker med hjälp av AI, så det enda som skickas är texten, och inte heller den sparas. Det du har sagt synkroniseras bara till din egen iCloud (privat).",
    ...
    "fi": {
        ...
        "privacy_body": "iPhonessa puheentunnistus tapahtuu kokonaan laitteellasi — KUU ei koskaan tallenna eikä lähetä ääntäsi. Apple Watchissa KUU käyttää kellon vakiosyöttöä ja vastaanottaa vain tekstin. Lajitteluun käytetään tekoälyä, joten laitteen ulkopuolelle lähetetään vain tekstiksi muutettu sisältö, eikä sitäkään säilytetä. Puhumasi asiat synkronoidaan vain omaan, yksityiseen iCloudiisi.",
    ...
    "fr": {
        ...
        "privacy_body": "Sur iPhone, l'écoute de votre voix se fait entièrement sur votre appareil : KUU ne conserve ni n'envoie jamais votre voix. Sur Apple Watch, KUU utilise la saisie standard de la montre et ne reçoit que du texte. Le tri utilise l'IA, donc seul le texte est envoyé, et il n'est pas conservé non plus. Ce que vous avez dit n'est synchronisé qu'avec votre iCloud privé.",
    ...
    "th": {
        ...
        "privacy_body": "บน iPhone การฟังเสียงเกิดขึ้นภายในเครื่องของคุณทั้งหมด KUU จะไม่เก็บหรือส่งเสียงของคุณออกไปไหนเลย บน Apple Watch นั้น KUU ใช้การป้อนข้อมูลมาตรฐานของนาฬิกา และรับเฉพาะข้อความเท่านั้น การจัดหมวดหมู่ใช้ AI ช่วย จึงส่งไปเฉพาะข้อความที่แปลงเป็นตัวอักษรแล้วเท่านั้น และก็ไม่ถูกเก็บไว้เช่นกัน สิ่งที่คุณพูดไปจะซิงก์ไปยัง iCloud ส่วนตัวของคุณเท่านั้น",
    ...
    "ru": {
        ...
        "privacy_body": "На iPhone распознавание голоса происходит полностью на вашем устройстве: KUU никогда не сохраняет и не отправляет сам голос. На Apple Watch KUU использует стандартный ввод часов и получает только текст. Для разбора мыслей используется ИИ, поэтому передаётся только текст — и он тоже не сохраняется. Сказанное вами синхронизируется только с вашим iCloud (приватным).",
