...
LOCALES = {
    "ja": {
        ...
        "privacy_body": "音声の聞きとりはすべて端末の中だけ。声そのものは残さず、送りません。整理には AI を使い、送るのは文字にした内容だけ。それも保存されません。話したことは、あなたの iCloud（プライベート）にだけ同期されます。",
        # Closing
    ...
    "en": {
        ...
        "privacy_body": "Listening happens entirely on your device — your voice is never kept or sent. Sorting uses AI, so only the text is sent, and it is never kept. Your notes sync only to your private iCloud.",
    ...
    "es": {
        ...
        "privacy_body": "Escuchar ocurre por completo en tu dispositivo: tu voz nunca se guarda ni se envía. La organización usa IA, así que solo se envía el texto, y este nunca se guarda. Tus notas se sincronizan solo con tu iCloud privado.",
    ...
    "ko": {
        ...
        "privacy_body": "듣기는 모두 기기 안에서만 이루어지며, 음성 자체는 남기거나 보내지 않습니다. 정리에는 AI를 사용하므로 텍스트만 보내며, 그것도 남지 않습니다. 말한 것은 당신의 iCloud(프라이빗)에만 동기화됩니다.",
    ...
    "zh-Hans": {
        ...
        "privacy_body": "聆听全都只在你的设备里完成——语音本身从不保留或发送。整理会使用 AI，因此只发送文字内容，而且不会保留。记录只同步到你私人的 iCloud。",
    ...
    "de": {
        ...
        "privacy_body": "Das Zuhören geschieht vollständig auf deinem Gerät. Deine Stimme selbst wird nicht gespeichert und nicht gesendet. Zum Sortieren kommt KI zum Einsatz, gesendet wird nur der verschriftlichte Inhalt – und auch der wird nicht gespeichert. Was du gesprochen hast, wird ausschließlich mit deiner privaten iCloud synchronisiert.",
    ...
    "it": {
        ...
        "privacy_body": "Il riconoscimento vocale avviene interamente sul dispositivo: la voce non viene conservata né inviata. Per l'ordinamento usiamo l'AI, quindi inviamo solo il testo trascritto, e nemmeno quello viene conservato. Ciò che dici si sincronizza solo con il tuo iCloud privato.",
    ...
    "vi": {
        ...
        "privacy_body": "Việc nhận diện giọng nói diễn ra hoàn toàn trong thiết bị. Giọng nói của bạn không được lưu lại và cũng không được gửi đi. Việc sắp xếp có dùng AI, nhưng chỉ gửi đi phần nội dung đã chuyển thành chữ, và phần đó cũng không được lưu. Những gì bạn nói chỉ được đồng bộ vào iCloud riêng tư của bạn.",
    ...
    "zh-Hant": {
        ...
        "privacy_body": "聆聽全都只在你的裝置裡完成——語音本身從不保留、也不會傳送。整理會用到 AI，因此只傳送轉成文字的內容，而且同樣不會保存。你說過的話，只會同步到你私人的 iCloud。",
    ...
    "nl": {
        ...
        "privacy_body": "Spraakherkenning gebeurt volledig op je eigen apparaat — je stem zelf wordt nooit bewaard of verzonden. Voor het sorteren gebruiken we AI, dus wordt alleen de tekst verzonden, en ook die wordt niet bewaard. Wat je hebt gezegd, wordt alleen gesynchroniseerd met jouw eigen, privé iCloud.",
    ...
    "id": {
        ...
        "privacy_body": "Pengenalan suara sepenuhnya terjadi di dalam perangkat Anda — suara Anda sendiri tidak pernah disimpan maupun dikirim. Untuk memilah, KUU memakai AI, sehingga yang dikirim hanya teks hasil ucapan, dan itu pun tidak disimpan. Apa yang Anda bicarakan hanya disinkronkan ke iCloud pribadi Anda.",
    ...
    "ms": {
        ...
        "privacy_body": "Pengecaman suara berlaku sepenuhnya di dalam peranti anda — suara itu sendiri tidak disimpan dan tidak dihantar. Penyusunan menggunakan AI, jadi hanya teks yang dihantar, dan itu juga tidak disimpan. Apa yang anda cakapkan hanya disegerakkan ke iCloud peribadi anda.",
    ...
    "da": {
        ...
        "privacy_body": "Lytningen foregår udelukkende på din enhed — din stemme bliver hverken gemt eller sendt nogen steder. Sorteringen bruger AI, så det er kun teksten, der bliver sendt, og heller ikke den bliver gemt. Det, du har sagt, synkroniseres udelukkende til din private iCloud.",
    ...
    "nb": {
        ...
        "privacy_body": "Talegjenkjenningen skjer helt og holdent på enheten din — stemmen din blir aldri lagret eller sendt noe sted. Sorteringen bruker KI, så det er bare teksten som sendes, og heller ikke den blir lagret. Det du har sagt, synkroniseres bare til din private iCloud.",
    ...
    "sv": {
        ...
        "privacy_body": "Taligenkänningen sker helt på den här enheten — själva rösten sparas eller skickas aldrig. Sortering sker med hjälp av AI, så det enda som skickas är texten, och inte heller den sparas. Det du har sagt synkroniseras bara till din egen iCloud (privat).",
    ...
    "fi": {
        ...
        "privacy_body": "Puheentunnistus tapahtuu kokonaan laitteellasi — ääntäsi ei koskaan tallenneta eikä lähetetä. Lajitteluun käytetään tekoälyä, joten laitteen ulkopuolelle lähetetään vain tekstiksi muutettu sisältö, eikä sitäkään säilytetä. Puhumasi asiat synkronoidaan vain omaan, yksityiseen iCloudiisi.",
    ...
    "fr": {
        ...
        "privacy_body": "L'écoute de votre voix se fait entièrement sur votre appareil : elle n'est jamais conservée ni envoyée. Le tri utilise l'IA, donc seul le texte est envoyé, et il n'est pas conservé non plus. Ce que vous avez dit n'est synchronisé qu'avec votre iCloud privé.",
    ...
    "th": {
        ...
        "privacy_body": "การฟังเสียงเกิดขึ้นภายในเครื่องของคุณทั้งหมด เสียงที่แท้จริงจะไม่ถูกเก็บไว้หรือส่งออกไปไหน การจัดหมวดหมู่ใช้ AI ช่วย จึงส่งไปเฉพาะข้อความที่แปลงเป็นตัวอักษรแล้วเท่านั้น และก็ไม่ถูกเก็บไว้เช่นกัน สิ่งที่คุณพูดไปจะซิงก์ไปยัง iCloud ส่วนตัวของคุณเท่านั้น",
    ...
    "ru": {
        ...
        "privacy_body": "Распознавание голоса происходит полностью на вашем устройстве: сам голос нигде не сохраняется и никуда не отправляется. Для разбора мыслей используется ИИ, поэтому передаётся только текст — и он тоже не сохраняется. Сказанное вами синхронизируется только с вашим iCloud (приватным).",
...
PRIVACY_META = {
    "ja": {
        ...
        "description": "KUU のプライバシーポリシー。音声はすべて端末内で処理し外部に送信しません。整理の AI 分類は既定で文字だけを送信し保存しません。",
    },
    "en": {
        "title": "Privacy Policy — KUU",
        "description": "KUU's privacy policy. Audio is processed entirely on device and never sent outside. AI organization sends only the text by default, and the AI provider does not retain it.",
    },
}
