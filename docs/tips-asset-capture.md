...
出力仕様: 860x798 h264 24fps + `<name>_poster.jpg`。撮影は `simctl io recordVideo` +
WDA actions でジェスチャー再現。**t_theme_drag だけはライブ再現不可** (上方向 reorder が
engage しない + system `.dropDestination` は synthetic touch で発火しない — PR #20 で
4 手法検証済み) のため、Place スクショ + ゴーストカードの合成アニメで作る。
詳細は PR #20 の記載と `capture_videos.py` (未整備なら PR #20 の手法欄) を参照。
